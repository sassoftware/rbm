...
class UrlSignTest(testcase.TestCase):
    ...
    def testUrlVerify(self):
        ...
        result = url_sign.verify_request(req)
        self.assertEqual(result, True)
        self.cfg.downloadSignatureKey = []
        self.assertRaises(RuntimeError, url_sign.verify_request, req)
    ...
    def testVerifyOldKey(self):
        ...
        req = self._req(path)
        self.assertEqual(url_sign.verify_request(req), True)
    ...
    def testVerifyBad(self):
        self.assertEqual(url_sign.verify_request(self._req('/example?e=1000003600')), False)
        self.assertEqual(url_sign.verify_request(self._req('/example?e=1000003600&s=')), False)
        path = url_sign.sign_path(self.cfg, '/example')
        path += '&e=2000000000'
        self._time(4000)
        self.assertEqual(url_sign.verify_request(self._req(path)), False)
